...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        init_len = len(sentence)
        
        if len(sentence) < max_length:
            
            sentence += [zero_vector] * (max_length - init_len)
            
            labels += [zero_label] * (max_length - init_len)
        ...
        mask = [True if index < init_len else False for index in range(max_length)]
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(shape = [None, self.max_length, Config.n_features],
                                                dtype = tf.int32)
        
        self.labels_placeholder = tf.placeholder(shape = [None, self.max_length],
                                                dtype = tf.int32)
        
        self.mask_placeholder = tf.placeholder(shape = [None, self.max_length],
                                                dtype = tf.bool)
        
        self.dropout_placeholder = tf.placeholder(shape = [],
                                                dtype = tf.float32)
    ...
    def add_embedding(self):
        ...
        embeddings_matrix = tf.constant(self.pretrained_embeddings)
        
        embeddings = tf.nn.embedding_lookup(embeddings_matrix, self.input_placeholder)
        
        embeddings = tf.reshape(embeddings, shape = [-1, self.max_length,
    ...
    def add_prediction_op(self):
        ...
        h = tf.zeros(shape = [batch_size, Config.hidden_size],
                     dtype = tf.float32)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o, h = cell(inputs, h, 'rnn_cell')
                
                o_drop = tf.nn.dropout(o, rate = 1 - dropout_rate)
                
                logits = tf.matmul(o_drop, U) + b_2
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.reduce_sum(tf.multiply(loss, mask), axis = 1))
