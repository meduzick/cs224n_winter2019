...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for index, word in enumerate(sentence):
            ...
            left_id = max(0, index - window_size)
    
            right_id = min(len(sentence) - 1, index + window_size)
                        
            add2left = 0 if index - window_size >= 0 else abs(index - window_size)
            
            add2right = (0 if index + window_size < len(sentence) \
                            else index + window_size - len(sentence) + 1)
            
            left_chunk = start * add2left
            
            right_chunk = end * add2right
            
            middle = sentence[left_id: right_id + 1]
            
            featured_word = left_chunk + [q for elem in middle for q in elem] + right_chunk
                        
            windowed_data.append((featured_word, labels[index]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(shape = [None, 
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embeddings_matrix, 
                                            self.input_placeholder)      
    ...
    def add_prediction_op(self):
        ...
        w = tf.get_variable('w',
                            shape = [self.config.n_window_features * self.config.embed_size,
        ...
        h_drop = tf.nn.dropout(h, rate = 1 - dropout_rate)
